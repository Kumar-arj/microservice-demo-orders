                ...
                container('docker') {
                    stage('Build Image') {
                        docker.withRegistry( 'https://registry.hub.docker.com', 'dockerhub' ) {
                            def customImage = docker.build('kumararj/sock-shop-micro-services-order:latest')
                            customImage.push()
                        }
                    }
                }
                ...
                container('build') {
                    dir('charts') {
                        withCredentials([usernamePassword(credentialsId: 'nexuslogin', usernameVariable: 'username', passwordVariable: 'password')]) {
                            ...
                            // '''
                            sh '/usr/local/bin/helm package micro-services-order'
                            ...
                            sh 'curl -v -u $username:$password --upload-file micro-services-order-1.0.tgz http://nexus.k4m.in/repository/sock-shop-helm-local/'
